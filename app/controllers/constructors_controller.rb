class ConstructorsController < ApplicationController
    ...
    def elo_rankings
        # Deduplicate: show only the highest-Elo constructor per family
        elo_col = Setting.elo_column(:elo)
        all_constructors = Constructor.where.not(elo_col => nil).order(elo_col => :desc).to_a
        seen_families = Set.new
        @constructors = all_constructors.select do |c|
          family = Constructor.family_for(c)
          if seen_families.include?(family)
            false
          else
    ...
    def best_pairings
        ...
        # Pre-load all race results grouped by (race_id, driver_id) for fast lookup
        all_results = RaceResult.includes(:race).index_by { |rr| [rr.race_id, rr.driver_id] }
        races_by_season = Race.joins(:race_results).distinct.group_by(&:season_id)

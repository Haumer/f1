class DriversController < ApplicationController
    def show
        ...
        @badges = @driver.badges.order(Arel.sql("CASE tier WHEN 'gold' THEN 0 WHEN 'silver' THEN 1 WHEN 'bronze' THEN 2 ELSE 3 END, id"))
    end
    ...
    def index
        ...
        if params[:search].present? && params[:search][:query].to_s.length > 1
            ...
            @drivers = base.where.not(peak_col => nil).order(peak_col => :desc)
        end
    end

...
class Fantasy::ReplayTransactionsTest < ActiveSupport::TestCase
  ...
  test "replays mixed roster and stock transactions in timestamp order" do
    sp = FantasyStockPortfolio.create!(
      user: @user, season: @season, cash: 0, starting_capital: 0
    )

    # Simulate: starting capital -> buy stock -> dividend
    @portfolio.transactions.create!(kind: "starting_capital", amount: 5000, note: "Starting capital", created_at: 1.hour.ago)
    ...
    # 5000 - 500 + 10 - 5 = 4505
    assert_equal 4505, entry[:new_cash]
  end

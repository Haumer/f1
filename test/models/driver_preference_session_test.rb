...
class DriverPreferenceSessionTest < ActiveSupport::TestCase
  ...
  test "top_five ranks final champion first, then losers in reverse elimination order" do
    ...
    ids = @session.top_five.map(&:id)
    assert_equal [@d.id, @a.id, @c.id, @b.id], ids
  end

  test "top_five demotes an early king who was later beaten" do
